...
const ProfileBasicDetailsTabs = () => {
  ...
  interface Clinic {
    ...
    operationalHours: OperationalHour[];
    clinicType: string; // e.g. "self" | "maia" | "other"
    // __v: number;
    // is_24_7?: boolean;
    // beds?: number;
    ...
    getLoggedInUser()
      .then((response) => {
        ...
        if (response.status == 200) {
          ...
          setServicesOffered(response.data.data.servicesOffered);
                ...
                <div className='treatment-steps-box mb-3' key={clinic._id}>
                    ...
                    <div className='d-flex gap-2'>
                      ...
                      <div className='d-flex flex-column'>
                        ...
                        <div className='d-flex align-items-center gap-2'>
                          ...
                          <span className="doctor-listing-modal-profile-subtitle">
                            {clinic.clinicType.charAt(0).toUpperCase() + clinic.clinicType.slice(1)}
                          </span>
                        </div>
                  ...
                  <Row className='mt-3'>
                    ...
                    <Col md={6}>
                      ...
                      {clinic.operationalHours && clinic.operationalHours.length > 0 ? (
                        <>
                          {/* --- MON TO FRI GROUP --- */}
                          <p className="patient-treatment-box-subtitle-desc m-0">
                            Mon to Fri:{" "}
                            {
                              clinic.operationalHours[0]?.openTime
                            }{" "}
                            -{" "}
                            {
                              clinic.operationalHours[0]?.closeTime
                            }
                          </p>
                          ...
                          <p className="patient-treatment-box-subtitle-desc m-0">
                            Sat & Sun:{" "}
                            {
                              clinic.operationalHours[5]?.openTime
                            }{" "}
                            -{" "}
                            {
                              clinic.operationalHours[5]?.closeTime
                            }
                          </p>
                        </>
                      ) : (
          ...
          <ContentContainer className="mt-4">
            ...
            <div className="d-flex gap-2 flex-wrap">
              {servicesOffered?.map((service, index) => (
                <div key={index} className="box-border-orange">
                  <span className="border-box-orange-font">{service}</span>
                </div>
              ))}
            </div>
