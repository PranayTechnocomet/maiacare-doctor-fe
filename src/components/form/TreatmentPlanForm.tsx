...
export function MedicationPrescriptionForm({
    ...
    {/*
        ...
        const handleAddNewMedication = (e: React.FormEvent) => {
            ...
            // If already showing the extra form, treat it like submit
            if (medicalPrescriptionDataShowHide) {
                ...
                if (Object.keys(errors).length === 0) {
                    ...
                    // Hide extra form after successful add
                    setMedicalPrescriptionDataShowHide?.(false);

                    console.log("New medication added from extra form");
                }
                return;
            }

            // If length < 2 → validate and add directly
            if (medicalPrescription.length < 2) {
                const errors = validateForm(formData);
                setFormError(errors);
                ...
                if (Object.keys(errors).length === 0) {
                    ...
                    console.log("New medication added directly");
                }
            } else {
                // If 2 or more → show form
                setMedicalPrescriptionDataShowHide?.(true);
            }
        };
     */}
