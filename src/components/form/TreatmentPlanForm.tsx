...
interface MedicationPrescriptionFormProps {
    ...
    TreatmentShowHideData?: boolean;
    setTreatmentShowHideData?: React.Dispatch<React.SetStateAction<boolean>>;
}
...
export function MedicationPrescriptionForm({
    ...
    medicalPrescription,
    TreatmentShowHideData,
    setTreatmentShowHideData
    ...
    const handleAddNewMedication = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (medicalPrescription.length == 2) {
            alert("call modal");
            // setTreatmentShowHideData?.(true);
        }

        if (Object.keys(errors).length === 0) {
            // console.log("medical prescription add data succesfully", formData);
           
            if (medicalPrescription.length === 1) {
                alert("call modal");
                ...
                console.log("medicalPrescription.length", medicalPrescription.length);
            }
    ...
    const handleSaveMedication = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (Object.keys(errors).length === 0) {
            if (editForm) {
                ...
                setTreatmentShowHideData?.(false);
    ...
    // console.log("TreatmentShowHideData", TreatmentShowHideData);
    
    return (
        <>
            <form onSubmit={handelNext}>
                {(medicalPrescription.length < 2 || TreatmentShowHideData == true) && (
                ...
                {editForm ?
                    <Button variant="outline" className="" onClick={handleSaveMedication}>
                        Save
                    </Button>
                    :
                    <>
                        ...
                        <div className="d-flex mt-3 gap-3">
                            <Button variant="outline" onClick={() => { setStep?.(step ? step - 1 : step); setStepper?.(stepper ? stepper - 1 : stepper); }} className="w-100">
                                <div className="d-flex justify-content-center align-items-center gap-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="15" viewBox="0 0 20 16" fill="none">
...
export function FollowUpActionForm({
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (Object.keys(errors).length === 0) {
            ...
            setTreatmentDetailsTempShow(true); // show static data state
        }
    };
